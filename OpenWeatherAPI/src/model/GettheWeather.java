...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		...
		String URLtoSend = "http://api.openweathermap.org/data/2.5/weather?q=" + wBean.getCityStr() + ","
				+ wBean.getCountryStr() + "&APPID=03b45293044f737cf9a009b723d136cd&mode=xml";
		...
		NodeList nList = doc.getElementsByTagName("clouds"); //how to add temperature?
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			Node nodeT = tList.item(temp);
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				
				// set the current node as an Element
				Element eElement = (Element) node;
				Element TElement = (Element) nodeT;
				...
				System.out.println(wBean.getCityStr() + " is now " + XMLclouds +" with a temperature of ");
				// save it
				wBean.setCloudsStr(XMLclouds);
				wBean.setTemp(XMLtemp);
